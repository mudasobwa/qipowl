...
Feature: Composer for HTML produces HTML
  ...
  Scenario Outline: Syntactic sugar
    ...
    Examples:
        ...
        | "http://mudasobwa.ru/images/am.jpg Standalone picture" | "<pclass='owl'><figure><imgsrc='http://mudasobwa.ru/images/am.jpg'/><figcaption><p>Standalonepicture</p></figcaption></figure></p>" |
  ...
  Scenario Outline: Syntactic sugar with carriage returns
    ...
    Examples:
        ...
        | "YouTube http://youtu.be/gokeLEC8dZc inline test." | "<p class='owl'>YouTube</p><iframe class='youtube' width='560' height='315' src='http://www.youtube.com/embed/gokeLEC8dZc'" |
  ...
  Scenario Outline: Linewides
    ...
    Examples:
        | input               | output                             |
        | "List: • li1 • li2" | "<p class='owl'>List:</p><ul><li>li1</li><li>li2</li></ul>" |
  ...
  Scenario Outline: Markdown atavisms ⇒ links
    ...
    Examples:
        ...
        | "![Figure](http://mudasobwa.ru/images/am.jpg)" | "<pclass='owl'><figure><imgsrc='http://mudasobwa.ru/images/am.jpg'/><figcaption><p>Figure</p></figcaption></figure></p>" |
  ...
  Scenario: Standalone images
    ...
    And parser is "html"
    When input string is processed with parser
    Then the result should equal to "<blockquote><divclass='blockquote'><figure><imgsrc='http://mudasobwa.ru/i/self.jpg'/><figcaption><p>Withcaption<br/><small><ahref='http://wikipedia.ru'>Wiki</a></small></p></figcaption></figure></div></blockquote><pclass='owl'>Nice?</p>"
...
    даже, если — печаль．
    ...
    And parser is "html"
    When input string is processed with parser
    Then the result should equal to "<pclass='owl'>  Да,яустал.Немногоесть.<br/> Да,лихорадитибросает.<br/>   Авгорлекомомбьетсяспесь.<br/> Отресторановдокрасавиц.<br/>Боюсьработы,дней,себя</p>"
...
"""
    ...
    Then the result should equal to
      """
<preclass='auto'>@@-348,7+348,7@@case"$DISTRO"in-rm-rf/usr/lib/nvidia-current/xorg/xorg+rm-rf/usr/lib/nvidia-current/xorg/xorg</pre><pclass='owl'><br/><small><ahref='https://github.com/MrMEEE/bumblebee/commit/a047be85247755cdbe0acce6#diff-1'>https://github.com/MrMEEE/bumblebee/commit/a047be85247755cdbe0acce6#diff-1</a></small></p>
      """
  ...
  Scenario: Weird behaviour with words starting with “ni”
    ...
    And rules from "lib/tagmaps/markdown2html.yaml" are merged in
    When input string is processed with parser
    Then the result should equal to "<p class='owl'>Very define and defined and even defined? words</p>"
